...
# Build the library
echo "Building png for $TARGET_ARCH..."
$NDK_PATH/ndk-build \
    ...
    APP_STL=c++_shared \
    APP_OPTIM=release \
    APP_BUILD_SCRIPT=$PROJECT_DIR/lib_pngdroid.mk

# Check for build success
if [ -f "$OUTPUT_DIR/$TARGET_ARCH/libpng.so" ]; then
    echo "Build successful! Library located at: $OUTPUT_DIR/$TARGET_ARCH/libpng.so"
else
