...
$NDK_PATH/ndk-build \
    ...
    APP_STL=c++_shared \
    APP_OPTIM=release \
    APP_BUILD_SCRIPT=$PROJECT_DIR/lib_jpegdroid.mk

# Check for build success
if [ -f "$OUTPUT_DIR/$TARGET_ARCH/libjpegdroid.so" ]; then
    echo "Build successful! Library located at: $OUTPUT_DIR/$TARGET_ARCH/libjpegdroid.so"
else
